    ...
    <Modal :title="modalTitle" v-model="roleModalVisible" :mask-closable='false' :width="500">
      <Form ref="roleForm" :model="roleForm" :label-width="80" :rules="roleFormValidate">
        <FormItem label="图片" prop="imgUrl">
          <qiniu
            @handleSuccess = "(url) => this.roleForm.imgUrl = url" :imgUrl="this.imageUrl">
          </qiniu>
        </FormItem>
        <FormItem label="排序值" prop="sortOrder">
          <Input v-model="roleForm.sortOrder" placeholder="排序值"/>
        </FormItem>
      </Form>
      <div slot="footer">
  ...
  export default {
          ...
          },
          {
            ...
            return h('div', {
              ...
              h('img', {
                ...
                attrs: {
                  src: params.row.imgUrl, style: 'width: 80px;height: 80px;border-radius: 2px;'
                },
                style: {
                },
              }),
    ...
    methods: {
      ...
      loadData() {
        ...
        this.getRequest("/goods/gallerys/", params).then(res => {
          this.loading = false;
        if (res.status === 200) {
      ...
      submitRole() {
        ...
        this.postBodyRequest("/goods/gallerys", this.roleForm).then(res => {
      ...
      edit(v) {
        ...
        this.roleForm.goodsId=this.$route.query.id,
          this.imageUrl=this.roleForm.imgUrl
          this.roleModalVisible = true;
      },
      remove(v) {
        this.$Modal.confirm({
          ...
          content: "您确认要删除图片?",
          onOk: () => {
          this.deleteRequest("/goods/gallerys", { id: v.id }).then(res => {
          if (res.status === 200) {
