        ...
        <Modal :title="modalTitle" v-model="roleModalVisible" :mask-closable='false' :width="500">
          <Form ref="serviceForm" :model="serviceForm" :label-width="80" :rules="serviceFormValidate">
            <FormItem label="关键字" prop="keyword">
              <Input v-model="serviceForm.keyword" placeholder="关键字"/>
            </FormItem>
            <FormItem label="类别" prop="type">
              <Input v-model="serviceForm.type" placeholder="类别"/>
            </FormItem>
            <FormItem label="内容" prop="content">
              <Input v-model="serviceForm.content" placeholder="内容"/>
            </FormItem>
            <FormItem label="服务接口" prop="serviceName">
              <Input v-model="serviceForm.serviceName" placeholder="服务接口"/>
            </FormItem>
          </Form>
          <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.getRequest("/messages", params).then(res => {
        this.loading = false;
        if (res.status == 200) {
    ...
    submitRole() {
      this.$refs.serviceForm.validate(valid => {
        if (valid) {
          ...
          this.postBodyRequest("/messages", this.serviceForm).then(res => {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.deleteRequest("/messages", { id: v.id }).then(res => {
            if (res.status == 200) {
    ...
    enable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/messages",{id:v.id,enable:1}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
    ...
    unEnable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/messages",{id:v.id,enable:0}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
