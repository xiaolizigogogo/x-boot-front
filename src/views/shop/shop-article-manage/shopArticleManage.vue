        ...
        <Modal :title="modalTitle" v-model="roleModalVisible" :mask-closable='false' :width="500">
          <Form ref="serviceForm" :model="serviceForm" :label-width="80" :rules="serviceFormValidate">
            <FormItem label="标题" prop="title">
              <Input v-model="serviceForm.title" placeholder="标题"/>
            </FormItem>
            <FormItem label="子标题" prop="subTitle">
              <Input v-model="serviceForm.subTitle" placeholder="子标题"/>
            </FormItem>
            <FormItem label="作者" prop="autor">
              <Input v-model="serviceForm.autor" placeholder="作者"/>
            </FormItem>
            <FormItem label="类型" prop="type">
              <Select v-model="serviceForm.type" placeholder="类型" clearable style="width: 200px">
            ...
            <FormItem label="列表首图" prop="listImageUrl">
              <qiniu
                @handleSuccess = "(url) => this.serviceForm.listImageUrl = url" :imgUrl="imageUrl">
              </qiniu>
            </FormItem>
            <FormItem>
              <texteditor :id="serviceForm.id+' '"></texteditor>
            </FormItem>
          </Form>
          <div slot="footer">
...
export default {
        ...
        },
        {
          ...
          return h('div', {
            ...
            h('img', {
              ...
              attrs: {
                src: params.row.listImageUrl, style: 'width: 159px;height: 60px;border-radius: 2px;'
              },
              style: {
              },
            }),
  ...
  methods: {
    ...
    submitRole() {
      this.$refs.serviceForm.validate(valid => {
        if (valid) {
          ...
          this.postBodyRequest("/articles", this.serviceForm).then(res => {
    ...
    edit(v) {
      ...
      this.imageUrl=this.serviceForm.listImageUrl+"?"+Math.random();
      this.roleModalVisible = true;
    },
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.deleteRequest("/articles", { id: v.id }).then(res => {
            if (res.status == 200) {
    ...
    enable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/articles",{id:v.id,enable:1}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
    ...
    unEnable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/articles",{id:v.id,enable:0}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
