<style lang="less">
@import "shopHomeMenuManage.less";
</style>
<template>
    <div class="search">
        <Row>
            <Col>
                <Card>
                  <Row>
                    ...
                    <Row class="operation">
                        <Button @click="addRole" type="primary" icon="plus-round">添加按钮</Button>
        ...
        <Modal :title="modalTitle" v-model="roleModalVisible" :mask-closable='false' :width="500">
          <Form ref="serviceForm" :model="serviceForm" :label-width="80" :rules="serviceFormValidate">
            ...
            <FormItem label="链接" prop="code">
              <Input v-model="serviceForm.targetUrl" placeholder="项目英文"/>
            </FormItem>
            ...
            <FormItem label="项目图片" prop="image">
              <qiniu
                @handleSuccess = "(url) => this.serviceForm.imageUrl = url" :imgUrl="imageUrl">
              </qiniu>
            </FormItem>
          </Form>
          <div slot="footer">
...
export default {
        ...
        },
        {
          ...
          return h('div', {
            ...
            h('img', {
              ...
              attrs: {
                src: params.row.imageUrl, style: 'width: 159px;height: 60px;border-radius: 2px;'
              },
              style: {
              },
            }),
  ...
  methods: {
    ...
    loadData() {
      ...
      this.getRequest("/homeMenus", params).then(res => {
        this.loading = false;
        if (res.status == 200) {
    ...
    submitRole() {
      this.$refs.serviceForm.validate(valid => {
        if (valid) {
          ...
          this.postBodyRequest("/homeMenus", this.serviceForm).then(res => {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.deleteRequest("/homeMenus", { id: v.id }).then(res => {
            if (res.status == 200) {
    ...
    enable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/homeMenus",{id:v.id,enable:1}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
    ...
    unEnable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/homeMenus",{id:v.id,enable:0}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
