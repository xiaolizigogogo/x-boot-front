<style lang="less">
@import "shopHomeActivitiManage.less";
</style>
<template>
    <div class="search">
        <Row>
            <Col>
                <Card>
                  <Row>
                    ...
                    <Row class="operation">
                        <Button @click="addRole" type="primary" icon="plus-round">添加活动</Button>
        ...
        <Modal :title="modalTitle" v-model="roleModalVisible" :mask-closable='false' :width="500">
          <Form ref="serviceForm" :model="serviceForm" :label-width="80" :rules="serviceFormValidate">
            ...
            <FormItem label="图片一" prop="image">
              <qiniu
                @handleSuccess = "(url) => this.serviceForm.containerBgUrl = url" :imgUrl="containerBgUrl">
              </qiniu>
            </FormItem>
            <FormItem label="图片一链接" prop="name">
              <Input v-model="serviceForm.containerBgTargetUrl" placeholder="项目中文名称"/>
            </FormItem>
            <FormItem label="图片二" prop="image">
              <qiniu
                @handleSuccess = "(url) => this.serviceForm.leftItemUrl = url" :imgUrl="leftItemUrl">
              </qiniu>
            </FormItem>
            <FormItem label="图片二链接" prop="name">
              <Input v-model="serviceForm.leftItemTargetUrl" placeholder="项目中文名称"/>
            </FormItem>
            <FormItem label="图片三" prop="image">
              <qiniu
                @handleSuccess = "(url) => this.serviceForm.rightItemFirstUrl = url" :imgUrl="rightItemFirstUrl">
              </qiniu>
            </FormItem>
            <FormItem label="图片三链接" prop="name">
              <Input v-model="serviceForm.rightItemFirstTargetUrl" placeholder="项目中文名称"/>
            </FormItem>
            <FormItem label="图片四" prop="image">
              <qiniu
                @handleSuccess = "(url) => this.serviceForm.rightItemSecondUrl = url" :imgUrl="rightItemSecondUrl">
              </qiniu>
            </FormItem>
            <FormItem label="图片四链接" prop="name">
              <Input v-model="serviceForm.rightItemSecondTargetUrl" placeholder="项目中文名称"/>
            </FormItem>
...
export default {
        ...
        },
        {
          ...
          return h('div', {
            ...
            h('img', {
              ...
              attrs: {
                src: params.row.containerBgUrl, style: 'width: 159px;height: 60px;border-radius: 2px;'
              },
              style: {
              },
            }),
  ...
  },
    {
      ...
      return h('div', {
        ...
        h('img', {
          ...
          attrs: {
            src: params.row.leftItemUrl, style: 'width: 159px;height: 60px;border-radius: 2px;'
          },
          style: {
          },
        }),
    ...
    },
    {
      ...
      return h('div', {
        ...
        h('img', {
          ...
          attrs: {
            src: params.row.rightItemFirstUrl, style: 'width: 159px;height: 60px;border-radius: 2px;'
          },
          style: {
          },
        }),
    ...
    },
    {
      ...
      return h('div', {
        ...
        h('img', {
          ...
          attrs: {
            src: params.row.rightItemSecondUrl, style: 'width: 159px;height: 60px;border-radius: 2px;'
          },
          style: {
          },
        }),
  ...
  methods: {
    ...
    loadData() {
      ...
      this.getRequest("/homeActivitis", params).then(res => {
        this.loading = false;
        if (res.status == 200) {
    ...
    submitRole() {
      this.$refs.serviceForm.validate(valid => {
          ...
          this.postBodyRequest("/homeActivitis", this.serviceForm).then(res => {
    ...
    addRole() {
      ...
      this.serviceForm={
        ...
        this.containerBgUrl=''
      this.leftItemUrl=''
        this.rightItemFirstUrl=''
        this.rightItemSecondUrl=''
    ...
    edit(v) {
      ...
      this.containerBgUrl=this.serviceForm.containerBgUrl+"?"+Math.random();
        this.leftItemUrl=this.serviceForm.leftItemUrl+"?"+Math.random();
        this.rightItemFirstUrl=this.serviceForm.rightItemFirstUrl+"?"+Math.random();
        this.rightItemSecondUrl=this.serviceForm.rightItemSecondUrl+"?"+Math.random();
      this.roleModalVisible = true;
    },
    remove(v) {
      this.$Modal.confirm({
    ...
    enable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/homeActivitis",{id:v.id,enable:1}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
    ...
    unEnable(v){
      this.$Modal.confirm({
        ...
        onOk: () => {
        this.postBodyRequest("/homeActivitis",{id:v.id,enable:0}).then(res => {
        if (res.status == 200) {
        this.$Message.success("操作成功");
        this.init();
      }
