<style lang="less">
@import "shopAnswerClassifyManage.less";
</style>
<template>
    <div class="search">
        <Row>
...
export default {
  name: "shop-answer-classify-manage",
  data() {
    return {
      loading: true,
      treeLoading: true,
      ...
      columns: [
        ...
        },
        {
          ...
          render:(h,params)=>{
            return h('div',
              params.row.gmtUpdate? formatDate(new Date(params.row.gmtUpdate)):"-");/*这里的this.row能够获取当前行的数据*/
          }
        },
        {
          title: "名称",
        ...
        },
        {
          ...
          align: "center",
          render:(h,params)=>{
            return h('div', this.deleteMap[params.row.payStatus]);/*这里的this.row能够获取当前行的数据*/
          }
        },
        {
          title: "创建人",
          ...
          render:(h,params)=>{
            return h('div',
              this.orderStatusMap[params.row.createBy]);/*这里的this.row能够获取当前行的数据*/
          }
        },
        {
          title: "修改人",
          ...
          render:(h,params)=>{
            return h('div',
              this.shippingStatus[params.row.updateBy]);/*这里的this.row能够获取当前行的数据*/
          }
        },
        {
          title: "版本",
  ...
  methods: {
    ...
    loadData() {
      ...
      this.getRequest("/nideshopAnswerClassifys", this.searchForm).then(res => {
        console.log(res)
        this.loading = false;
        if (res.status === 200) {
